...
itself** (`github.com/sailpoint-oss/golang-sdk` — historically the `/v2`
line's `api_beta`/etc. subpackages, and as of the 2026-08-02 migration the
`/v3` line's per-service packages like `sources`/`roles`/`workflows`) —
struct typing, missing methods, inconsistent shapes across
...
- **Status:** **Resolved in golang-sdk v3** (confirmed 2026-08-02 by direct source inspection of `v3.1.10`). Was Open through the entire v2 line. Dead-but-retained workaround still shipped on v3 (see below).
- **Package/type:** v2 `api_beta.ProvisioningConfigManagedResourceRefsInner` (file `model_provisioning_config_managed_resource_refs_inner.go`); in v3 the type was renamed/restructured to `service_desk_integration.ServiceDeskSource`.
- **Affected versions confirmed via direct source inspection:** `v2.5.1` (this repo's original pin), `v2.7.81`, `v2.7.106` (last v2 release) — a v2 version bump alone never fixed it. **`v3.1.10` FIXES it**: `service_desk_integration.ServiceDeskSource` declares `Type/Id/Name` as `*string` (and `ProvisioningConfig.ManagedResourceRefs` is now `[]ServiceDeskSource`), so the SDK's own `Execute()` unmarshals populated `managedResourceRefs` cleanly.
...
- **Package/type:** `JsonPatchOperation.Value`, a oneOf wrapper shared across every JSON-Patch-based update endpoint in the SDK. In v2 (`api_beta`) it was misleadingly typed `*UpdateMultiHostSourcesRequestInnerValue` (named after one specific unrelated endpoint). **v3 renamed this wrapper to the sensible `JsonPatchOperationValue`** (and its `StringAs.../BoolAs.../MapmapOfStringAnyAs.../ArrayOfArrayInnerAs...` constructor prefixes follow the new name), which is a genuine ergonomic improvement — though the wrapper-not-plain-`interface{}` integration cost itself remains.
...
- **Status:** **Resolved in golang-sdk v3** (confirmed 2026-08-02). Was a v2 cosmetic outlier. v3's per-service-package + universal-`V1`-suffix naming convention renamed it to the consistent `sources.SourcesAPIService.DeleteSourceV1`, so it's no longer an outlier.
- **Package/type:** v2 `api_beta.SourcesAPIService` — its delete operation was generated as plain `Delete(ctx, id)`, unlike most other v2 `*APIService` types which consistently named their delete method `Delete<Resource>` (e.g. `DeleteGovernanceGroup`, `DeleteRole`, `DeleteAccessProfile`). In v3 it is `DeleteSourceV1`.
...
- **Workaround (shipped):** always pass a real (even throwaway/empty) `*os.File` rather than `nil` when calling an affected method — confirmed working for `ImportEntitlements` in `entitlement_v1`. Applied defensively (not yet individually live-confirmed) to `ImportAccountsV1` too in `source_actions_v1`'s `identitynow_aggregate_accounts` action, alongside `ImportEntitlementsV1` in its `identitynow_aggregate_entitlements` action.
- **Filed upstream?** No.
...
- **Status:** **Resolved in golang-sdk v3** (confirmed 2026-08-02 by direct source inspection of `v3.1.10`). Was Open through the entire v2 line. `source_provisioning_policy_v1` still ships on the v1 endpoints pending a live-verified migration (see revisit trigger).
- **Package/type:** v2 `api_beta.SourcesAPIService` — confirmed via direct grep of `api_sources.go` (through `v2.7.106`) that only v1's `CreateProvisioningPolicy`/`GetProvisioningPolicy`/`UpdateProvisioningPolicy`/`PutProvisioningPolicy`/`DeleteProvisioningPolicy`/`ListProvisioningPolicies` (all keyed by `sourceId`+`usageType`) existed. **v3's `sources.SourcesAPIService` now DOES publish** `CreateProvisioningPolicyV2`/`GetProvisioningPolicyV2`/`PutProvisioningPolicyV2`/`DeleteProvisioningPolicyV2` (alongside the `...V1` set), each with a generated request-builder `XSailPointExperimental(bool)` setter — so both the missing-methods blocker AND the can't-attach-`X-SailPoint-Experimental`-header blocker are lifted.
...
- **Filed upstream?** No.
- **Revisit trigger:** if a future `golang-sdk` release adds v2 provisioning-policy bindings (confirmed via direct source inspection, not changelog alone) AND a way to attach the `X-SailPoint-Experimental` header, revisit `source_provisioning_policy_v1` to migrate to `id`-keyed v2 CRUD — a strictly better fit for Terraform's resource model (no composite key, no synthesized id). **UPDATE 2026-08-02: both conditions are now met in v3** (`...ProvisioningPolicyV2` methods + per-request `XSailPointExperimental` setter). Migrating the resource to by-id v2 CRUD is now a viable, tracked follow-up — deferred out of the static-only v3 migration because it changes the resource's Terraform id shape (composite `<source_id>/<usage_type>` → real `id`), which is a breaking state change that needs live-tenant verification and a considered upgrade/migration story, not a mechanical swap.
- **Discovered:** 2026-08-01, `source_provisioning_policy_v1` pipeline. **Resolution confirmed:** 2026-08-02, golang-sdk v3 migration.

### 11. `ListWorkflowsV1`'s generated request builder dropped ALL query params in v3 (regression)
- **Status:** Open (upstream regression, introduced by golang-sdk v3). Data source scope reduced (see below).
- **Package/type:** `workflows.WorkflowsAPIService.ListWorkflowsV1` / its `ApiListWorkflowsV1Request` — confirmed via direct source inspection of `v3.1.10` that the request builder has **zero** query-param setter methods (no `Filters`/`Sorters`/`Offset`/`Limit`), only `ctx`/`Execute()`. The v2 equivalent (`api_beta.WorkflowsAPI.ListWorkflows`) **did** expose these builders.
- **Impact:** `identitynow_workflows_v1` (plural data source) can no longer push `filters`/`sorters`/`offset`/`limit` server-side under v3. Structurally identical to issue #9 (`GetConnectorRuleList`) — a spec-declares-params-but-SDK-omits-builders gap — but here it's a *regression* from a version where the builders existed, not a never-had-it gap.
- **Workaround (shipped):** `datasource_workflows.go` now emits a Warning diagnostic when `filters`/`sorters`/`offset` are set (they're silently unsupported server-side under v3) and applies `limit` **client-side** by truncating the full result slice after fetch. Functionally correct for small/medium tenants; large tenants lose server-side filtering efficiency.
- **Filed upstream?** No.
- **Revisit trigger:** if a later v3 release restores the `ListWorkflowsV1` request-builder query params (confirmed via direct source inspection), remove the client-side truncation + warning and wire the builders back up, mirroring how the other plural data sources push filters server-side.
- **Discovered:** 2026-08-02, golang-sdk v3 migration (`workflow_v1`).
