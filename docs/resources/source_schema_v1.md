...
- **Phase B (live `terraform plan`/`apply` against a real sandbox tenant)
  ...
  deleting a schema that's still referenced by another schema's
  `attributes[].schema` pointer is rejected by the live API (this
  resource's `Delete` does not pre-check for that), and whether any
  Computed-only attribute needs a `UseStateForUnknown` plan modifier
  beyond what's already applied here (see
  `resource_source_schema_planmodifiers.go`).
