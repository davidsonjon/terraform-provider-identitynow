...
This resource's schema/model types for most attributes are generated by
`tfplugingen-framework` from SailPoint's per-service `/workflows/v1`
OpenAPI spec, but `definition` and the entire `trigger` block are
hand-written (not generated), and Create/Read/Update/Delete are hand-written
against `golang-sdk/v3`'s `workflows.WorkflowsAPIService` client.

- **Scope: only core workflow CRUD is implemented.** The following related
  Workflows API operations are deliberately out of scope for this resource
  (no resource/data source manages them):
